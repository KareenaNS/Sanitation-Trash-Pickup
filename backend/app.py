...
# #     try:
# #         # Update the resident's trash collection status in Firestore
# #         resident_ref = db.collection('residents').document(resident_id)
# #         resident_ref.update({
# #             'trashCollection': True  # Set trashCollection to true
# #         })
...
# #     try:
# #         resident_ref = db.collection('residents').document(resident_id).get()
# #         if resident_ref.exists:
# #             resident_data = resident_ref.to_dict()
# #             return jsonify({
# #                 'message': 'Resident found',
# #                 'paymentStatus': resident_data.get('paymentStatus'),
# #             }), 200
# #         else:
...
def create_resident():
                ...
                with open(filename, 'r') as csvfile:
                    ...
                    for row in reader:
                        ...
                        resident_data = {
                            ...
                            'trashCollection': trash_collection.lower() == 'yes',  # Convert to boolean
                            # 'qrCodeData': f"https://api.qrserver.com/v1/create-qr-code/?data={resident_id}&size=150x150"  # QR code URL
                            'qrCodeData': f"https://api.qrserver.com/v1/create-qr-code/?data=http://localhost:3000/payment-status/{resident_id}&size=150x150"  # QR code URL
                        }
                        db.collection('residents').document(str(resident_id)).set(resident_data)
            except Exception as e:
                return jsonify({'error': str(e)}), 500
    ...
    if data:
        ...
        resident_data = {
            ...
            'paymentStatus': payment_status.lower() == 'yes',  # Convert to boolean
            'trashCollection': trash_collection.lower() == 'yes',  # Convert to boolean
            'qrCodeData': f"https://api.qrserver.com/v1/create-qr-code/?data=http://192.168.1.7:5000/get-resident-status/{resident_id}&size=150x150"  # QR code URL
        }
        db.collection('residents').document(str(resident_id)).set(resident_data)
        return jsonify({'id': resident_id, 'message': 'Resident created successfully!'}), 201
    return jsonify({'error': 'No data provided'}), 400
